...
def cell_attached(frame, i, zxsec, rthresh, engine=plt, **kwargs):
    """
...
def cells_attached(frame, zxsec, rthresh,engine=plt, colors='k'):
    """
    Plot all cells as contours.
    ...
    for i in range(len(frame.phi)):
        cell_attached(frame, i, zxsec,rthresh, engine, color='k')
...
def fill_cell(frame, lprop,i,zxsec,engine=plt):

    p = frame.phi[i]
    p = p[zxsec,:,:]

    color = ['#edeef5','#FFC300','#FF5733','#bf0823','#581845']
    #color = ['#FFC300','#FF5733','#bf0823','#581845']
    ...
    engine.contourf(np.arange(0, frame.parameters['Size'][1]),
                   np.arange(0, frame.parameters['Size'][0]),
                   p, linewidths =[0.5],colors=color[int(lprop[i]-np.min(lprop)+1)],levels=[0.5,1.5])
                   
def fill_cell_prop(frame,lprop, zxsec, engine=plt ):
	for i in range(len(frame.phi)):
		fill_cell(frame,lprop,i,zxsec, engine) 
...
def fill_cell_freud(frame, lprop,i,zxsec,color,engine=plt):
    ...
    engine.contourf(np.arange(0, frame.parameters['Size'][1]),
                   np.arange(0, frame.parameters['Size'][0]),
                   p, linewidths =[0.5],colors=color,levels=[0.5,1.5])
                   
def fill_cell_prop_freud(frame,lprop, zxsec, engine=plt ):

	#norm = plt.Normalize()
	colors = plt.cm.jet
	cprop = lprop/np.max(lprop)
	#print(cprop)

	for i in range(len(frame.phi)):
		ccolor = matplotlib.colors.to_hex(colors(cprop[i]))
		#print(ccolor)
		fill_cell_freud(frame,lprop,i,zxsec,ccolor, engine) 
...
def coarse_grain_stress_field(f1,cgLz,cgLy,cgLx,size,engine=plt):
            ...
            if (abs(w[i,j]) >= 0.49 and abs(w[i,j]) < 0.51):
                ...
                for ii in (rngA):
                    for jj in (rngA):
                        tripleti = ((i_n+ii) % (LY)) 
                        tripletj = ((j_n+jj) % (LX)) 
                        if ((ql * w[tripleti-1,tripletj-1] > 0.49) and (ql * w[tripleti-1,tripletj-1]) < 0.51 ):
                            yy = yy + inew - 1.
                            xx = xx + jnew - 1.
                            n1 = n1 + 1
...
def defects(dx,dy, engine=plt, arrow_len=8):
    ...
    for d in defects:
        if d['charge'] == 0.5:
            engine.plot(d["pos"][0], d["pos"][1], 'bo', markersize=8)
        elif d['charge'] == -0.5:
            engine.plot(d["pos"][0], d["pos"][1], 'g>', markersize=8)

        elif d['charge'] == 1.:
            engine.plot(d["pos"][0], d["pos"][1], 'k*', markersize=8)

        elif d['charge'] == -1.:
            engine.plot(d["pos"][0], d["pos"][1], 'ys', markersize=8)
...
def get_defects_2D_cg(w,xcoor,ycoor):
